...
var Media = function (_Component) {
  ...
  function Media() {
    ...
    return _ret = (_temp = (_this = _possibleConstructorReturn(this, (_ref = Media.__proto__ || Object.getPrototypeOf(Media)).call.apply(_ref, [this].concat(args))), _this), _this.state = {
      ...
      _this._playerProps = _extends({}, _this._playerProps, props);
      ...
      _this._setState({ currentTime: currentTime });
    }, _this.skipTime = function (amount) {
      var _this$_getState = _this._getState(),
          currentTime = _this$_getState.currentTime,
          duration = _this$_getState.duration;
      ...
      if (isMuted) {
        _this._lastVolume = _this._getState().volume;
      ...
      _this.mute(!_this._getState().isMuted);
    }, _this.setVolume = function (volume) {
      var isMuted = volume <= 0;

      if (isMuted !== _this._getState().isMuted) {
        _this.mute(isMuted);
      } else {
      ...
      var newVolume = _this._getState().volume + amount * 0.01;
      ...
      if (!_this._getState().isFullscreen) {
      ...
      if (target === _this._player.node) {
        _this._setState({ isFullscreen: !_this._getState().isFullscreen });
      }
  ...
  _createClass(Media, [{
    ...
    value: function _getPublicMediaProps() {
      return _extends({}, this._getState(), {
        play: this.play,
        pause: this.pause,
    ...
    value: function _getPlayerEvents() {
      ...
      MEDIA_EVENTS_KEYS.forEach(function (key) {
        ...
        var leadingPropCallback = _this2._playerProps['_' + key + 'Leading'];

        events[key] = function (val) {
          if (stateKey) {
            if (!_this2.unmounted) {
              _this2._setState(_defineProperty({}, stateKey, val));
            }
            var newState = _extends({}, _this2._getState(), _defineProperty({}, stateKey, val));
            if (_this2.context._eventCallback) {
              _this2.context._eventCallback(key, _this2, newState);
            }
...
Media.contextTypes = _containerContextTypes2.default;
